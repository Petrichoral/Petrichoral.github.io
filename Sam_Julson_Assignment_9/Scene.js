...
const octoupdate = (function() {
  ...
  return (function(modelViewMatrix, deltaTime) {
    ...
    return twgl.m4.scale(
      modelViewMatrix,
      [Math.abs(Math.cos(angle)), Math.abs(Math.cos(angle)), Math.abs(Math.cos(angle))],
      modelViewMatrix,
    );
  });
})();
...
grobjects.push(Grass("grass-plane", {translate: [0, -10, 0]}, null));
grobjects.push(Sky("Skybox", null, null));